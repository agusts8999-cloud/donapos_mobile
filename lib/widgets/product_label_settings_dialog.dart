...
class ProductLabelSettingsDialog extends StatefulWidget {
  const ProductLabelSettingsDialog({super.key});

  @override
  State<ProductLabelSettingsDialog> createState() => _ProductLabelSettingsDialogState();
}

class _ProductLabelSettingsDialogState extends State<ProductLabelSettingsDialog> {
  // Data
  ...
  void initState() {
    super.initState();
    debugPrint("[ProductLabel] initState called");
  ...
  Future<void> _refreshData() async {
    debugPrint("[ProductLabel] _refreshData started");
    ...
    try {
      ...
      debugPrint("[ProductLabel] _refreshData finished, prods: ${_products.length}");
    } catch (e) {
      debugPrint("[ProductLabel] Error: $e");
  ...
  Future<void> _updateLabelStatus(int id, int current) async {
    final newVal = current == 1 ? 0 : 1;
    setState(() {
  ...
  Widget build(BuildContext context) {
    debugPrint("[ProductLabel] build called, loaded: $_isDataLoaded");
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: FittedBox(
                      child: Switch(value: _labelPrinterEnabled, activeColor: MetroColors.primary, onChanged: (v) async {
                         ...
                         p.setBool('label_printer_enabled', v);
                      }),
                    ),
                  ),
                  const SizedBox(width: 8),
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      onPressed: () async {
                        try {
                          await showDialog(context: context, builder: (_) => const LabelConnectionDialog());
                          if (mounted) _loadSettings();
                        } catch (e) {
                          debugPrint("[ProductLabel] Scanner error: $e");
                        }
                      },
                          ...
                          SizedBox(
                            ...
                            child: FittedBox(
                              child: Switch(
                                ...
                                onChanged: (v) => _updateLabelStatus(p['id'], p['needs_label'] ?? 0),
                              ),
                            ),
                          )
                       ],
...
class LabelConnectionDialog extends StatefulWidget {
  const LabelConnectionDialog({super.key});
  @override
  State<LabelConnectionDialog> createState() => _LabelConnectionDialogState();
}

class _LabelConnectionDialogState extends State<LabelConnectionDialog> {
  ...
  Widget build(BuildContext context) {
    return Dialog(
       ...
       child: Container(
         ...
         child: Column(
           children: [
             const Text("PILIH PRINTER", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.black87)),
             const Divider(),
             Expanded(
               child: _isLoading 
                 ? const Center(child: CircularProgressIndicator()) 
